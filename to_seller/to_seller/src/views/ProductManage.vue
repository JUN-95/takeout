...
<script>
    export default {
        ...
        methods:{
            page(currentPage){
                const _this = this
                axios.get('http://localhost:8383/product-service/seller/product/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
            ...
            del(row) {
                ...
                this.$confirm('确认删除【'+row.name+'】吗？', '提示', {
                    ...
                    axios.delete('http://localhost:8383/product-service/seller/product/delete/'+row.id).then(function (resp) {
                        if(resp.data.code==0){
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8383/product-service/seller/product/list/1/'+_this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data.content
                _this.pageSize = resp.data.data.size
                _this.total = resp.data.data.total
            })
