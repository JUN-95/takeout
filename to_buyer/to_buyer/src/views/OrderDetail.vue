...
<script>
    ...
    export default {
        ...
        created(){
            ...
            const _this = this
            axios.get('http://localhost:8383/order-service/buyer/order/detail/'+JSON.parse(window.localStorage.getItem('access-user')).openid+'/'+this.$route.query.orderId).then(function (resp) {
        ...
        methods:{
            ...
            cancelOrder(){
                ...
                MessageBox.confirm('确定取消订单吗?').then(action => {
                    axios.put('http://localhost:8383/order-service/buyer/order/cancel/'+JSON.parse(window.localStorage.getItem('access-user')).openid+'/'+this.$route.query.orderId).then(function (resp) {
                        if(resp.data.code == 0){
